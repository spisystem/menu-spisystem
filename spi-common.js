...
const SPI_COOKIE_TEXTS = {
  it:{
    ...
    accept:"Accetta",decline:"Rifiuta",customize:"Personalizza",
    ...
    save:"Salva preferenze",privacyLink:"Leggi la Privacy Policy"
  },
  en:{
    ...
    accept:"Accept",decline:"Decline",customize:"Customize",
    ...
    save:"Save preferences",privacyLink:"Read Privacy Policy"
  },
  es:{
    title:"Tu privacidad importa",
    text:"Solo usamos cookies técnicas necesarias para el funcionamiento del sitio. No usamos cookies de rastreo o publicidad.",
    accept:"Aceptar",decline:"Rechazar",customize:"Personalizar",
    ...
    save:"Guardar preferencias",privacyLink:"Leer Política de Privacidad"
  },
  fr:{
    title:"Votre vie privée compte",
    text:"Nous utilisons uniquement des cookies techniques nécessaires au fonctionnement du site. Aucun cookie de suivi ou publicitaire.",
    accept:"Accepter",decline:"Refuser",customize:"Personnaliser",
    ...
    save:"Enregistrer",privacyLink:"Lire la Politique de Confidentialité"
  },
  de:{
    title:"Ihre Privatsphäre ist wichtig",
    text:"Wir verwenden nur technische Cookies, die für die Funktion der Website notwendig sind. Keine Tracking- oder Werbe-Cookies.",
    accept:"Akzeptieren",decline:"Ablehnen",customize:"Anpassen",
    ...
    save:"Einstellungen speichern",privacyLink:"Datenschutzerklärung lesen"
  }
};
...
function spiGetLang(){ return localStorage.getItem('selectedLanguage')||'it'; }
function spiCurrentCookieTexts(){ return SPI_COOKIE_TEXTS[spiGetLang()]||SPI_COOKIE_TEXTS.it; }

function spiRefreshCookieBannerTexts(){
  const banner=document.getElementById('spiCookieBanner'); if(!banner)return;
  ...
  p.innerHTML=`${t.text} <a href="privacy.html">${t.privacyLink}</a>`;
...
function initCookieBanner(){
  if(localStorage.getItem('spi-cookie-consent'))return;
  const t=spiCurrentCookieTexts();
  ...
  banner.innerHTML=`
    <div class="spi-cookie-card">
      ...
      <div class="spi-cookie-body">
        <h3>${t.title}</h3>
        <p>${t.text} <a href="privacy.html">${t.privacyLink}</a></p>
      </div>
  ...
  function closeBanner(){ banner.classList.remove('show'); setTimeout(()=>banner.remove(),350); }
...
function openSettingsModal(t){
  ...
  const existing=document.getElementById('spiCookieSettings'); if(existing)existing.remove();
  ...
  modal.innerHTML=`
    <div class="spi-cookie-modal-card">
      ...
      <div class="spi-cookie-row">
        ...
        <label class="spi-switch disabled"><input type="checkbox" checked disabled><span class="spi-slider"></span></label>
      </div>
      <div class="spi-cookie-row">
        <div><strong>${t.analytics}</strong><p>${t.analyticsDesc}</p></div>
        <label class="spi-switch"><input type="checkbox" id="spiAnalyticsToggle"><span class="spi-slider"></span></label>
      </div>
